...
func testHelper(t *testing.T, data *TestData, podAIPs, podBIPs, podCIPs, podDIPs, podEIPs *PodIPs, isIPv6 bool) {
	_, _, err := createPerftestServices(data, isIPv6)
	if err != nil {
	...
	t.Run("IntraNodeDenyConnNP", func(t *testing.T) {
		...
		trafficInfo := traffic{
			srcPodName1: "perftest-a",
			srcPodName2: "perftest-d",
			dstPodName1: "perftest-b",
			dstPodName2: "perftest-a",
		}
		if !isIPv6 {
			trafficInfo.srcIP1, trafficInfo.srcIP2, trafficInfo.dstIP1, trafficInfo.dstIP2 = podAIPs.ipv4.String(), podDIPs.ipv4.String(), podBIPs.ipv4.String(), podAIPs.ipv4.String()
			checkRecordsForDenyFlows(t, data, trafficInfo, isIPv6, true, false)
		} else {
			trafficInfo.srcIP1, trafficInfo.srcIP2, trafficInfo.dstIP1, trafficInfo.dstIP2 = podAIPs.ipv6.String(), podDIPs.ipv6.String(), podBIPs.ipv6.String(), podAIPs.ipv6.String()
			checkRecordsForDenyFlows(t, data, trafficInfo, isIPv6, true, false)
		}
	})
	//
...
func checkRecordsForDenyFlows(t *testing.T, data *TestData, trafficInfo traffic, isIPv6 bool, isIntraNode bool, isANP bool) {
	timeStart := time.Now()
	var cmdStr1, cmdStr2 string
	if !isIPv6 {
		cmdStr1 = fmt.Sprintf("iperf3 -c %s -n 1", trafficInfo.dstIP1)
		cmdStr2 = fmt.Sprintf("iperf3 -c %s -n 1", trafficInfo.dstIP2)
	} else {
	...
	for _, record := range recordSlices {
		...
		if strings.Contains(record, trafficInfo.srcIP1) && strings.Contains(record, trafficInfo.dstIP1) {
			srcPodName = trafficInfo.srcPodName1
			dstPodName = trafficInfo.dstPodName1
		} else if strings.Contains(record, trafficInfo.srcIP2) && strings.Contains(record, trafficInfo.dstIP2) {
			srcPodName = trafficInfo.srcPodName2
			dstPodName = trafficInfo.dstPodName2
		}
		if (strings.Contains(record, trafficInfo.srcIP1) && strings.Contains(record, trafficInfo.dstIP1)) || (strings.Contains(record, trafficInfo.srcIP2) && strings.Contains(record, trafficInfo.dstIP2)) {
			...
			if !isANP { // K8s Network Policies
				if strings.Contains(record, ingressDropStr) && !strings.Contains(record, ingressDropANPName) {
					assert.Contains(t, record, trafficInfo.dstIP1)
				} else if strings.Contains(record, egressDropStr) && !strings.Contains(record, egressDropANPName) {
					assert.Contains(t, record, trafficInfo.dstIP2)
				}
			} else { // Antrea Network Policies
				if strings.Contains(record, ingressRejectStr) {
					if !strings.Contains(record, ingressRejectANPName) {
						t.Errorf("Record does not have Antrea NetworkPolicy name with ingress reject rule")
					}
					if !strings.Contains(record, fmt.Sprintf("%s: %s", "ingressNetworkPolicyNamespace", testNamespace)) {
						t.Errorf("Record does not have correct ingressNetworkPolicyNamespace")
					}
				} else if strings.Contains(record, ingressDropStr) {
					if !strings.Contains(record, ingressDropANPName) {
						t.Errorf("Record does not have Antrea NetworkPolicy name with ingress drop rule")
					}
					if !strings.Contains(record, fmt.Sprintf("%s: %s", "ingressNetworkPolicyNamespace", testNamespace)) {
						t.Errorf("Record does not have correct ingressNetworkPolicyNamespace")
					}
				} else if strings.Contains(record, egressRejectStr) {
					if !strings.Contains(record, egressRejectANPName) {
						t.Errorf("Record does not have Antrea NetworkPolicy name with egress reject rule")
					}
					if !strings.Contains(record, fmt.Sprintf("%s: %s", "egressNetworkPolicyNamespace", testNamespace)) {
						t.Errorf("Record does not have correct egressNetworkPolicyNamespace")
					}
				} else if strings.Contains(record, egressDropStr) {
					if !strings.Contains(record, egressDropANPName) {
						t.Errorf("Record does not have Antrea NetworkPolicy name with egress drop rule")
					}
					if !strings.Contains(record, fmt.Sprintf("%s: %s", "egressNetworkPolicyNamespace", testNamespace)) {
						t.Errorf("Record does not have correct egressNetworkPolicyNamespace")
					}
